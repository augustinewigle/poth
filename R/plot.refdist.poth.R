...
#'
#' @param x R object of class \code{refdist.poth}.
#' @param include.observed Logical; Should a line with the observed POTH be drawn on the plot? Default is TRUE
#' @param include.probability Logical; Should the probability of a POTH greater than the observed POTH be printed in the legend? Default is F.
#'        Ignored if include.observed = F.
#' @param \dots Additional arguments (ignored).
#'
#' @details
#' Plots a histogram of the simulated POTH values from the reference distribution.
#' (Wigle et al., 2024).
#'
#' @return
#' A ggplot2 object.
...
#' plot(dist1, include.observed = FALSE)
#'
#' \donttest{
#' data(Senn2013)
...
#' plot(dist2, include.probability = TRUE)
#' }
#' @method plot refdist.poth
#' @export
...
plot.refdist.poth <- function(x, include.observed = TRUE, include.probability = FALSE, ...) {
  ...
  if(include.observed) {
    ...
    if(include.probability) {
      ...
      dat2 <- data.frame(obs = c(p$poth, 0),
                         name2 = c(paste0("Observed\nPOTH = ", round(p$poth, 3)),
                                   paste0("Pr(POTH > ", round(p$poth, 3), ") = ", round(mean(x$dist>p$poth), 3))))

    } else {

      dat2 <- data.frame(obs = p$poth,
  ...
  g <- ggplot(data = dat,
              aes(x = dist)) +
    geom_histogram(col = "black", fill = "lightgrey", binwidth = (max(x$dist)-min(x$dist))/30) +
    theme_bw() +
  ...
  if(include.observed) {
    ...
    g <- g +
      geom_vline(data = dat2, aes(xintercept = obs, color = name2),  show.legend = T, linetype = "dashed", linewidth = 1) +
